...
\begin{document}
	...
	\newpage
	\subsection{Degree, Homological Orientation and Further Applications}
	...
	\begin{sketch}
		\begin{minipage}[t]{\linewidth-3cm}
			...
			\begin{equation*}
				\im p_i \intersection \im p_j = \convex \{v_0,...,v_i,w_{j+1}...,w_n\}.
			\end{equation*}
	...
	\begin{proof}
		...
		Let $s: \mathbb{\Delta}^n --> X$ be a singular $n$-simplex. From the linear $(n+1)$-simplices $p_i: \mathbb{\Delta}^{n+1} --> I \times \mathbb{\Delta}^n$ used in subdividing the prism we obtain a family  of singular $(n+1)$-simplices $q_i:\mathbb{\Delta}^{n+1} --> Y$ by taking the composite
		\begin{equation*}
			\begin{diagram}
				\fourbyone[wide]
					{\mathbb{\Delta}^{n+1}}{I \times \mathbb{\Delta}^n}{I \times X}{Y.}

				\arrow{ww}{w}{p_i}[above]
				\arrow{w}{e}{I \times s}[above]
				\arrow{e}{ee}{h}[above]
			\end{diagram}
		\end{equation*}
		Define the map $k_n: C_n^\text{sing}(X) \rightarrow C_{n+1}^\text{sing}(Y)$ by linear extension of the assignment
		\begin{equation*}
			k_n(e_s) = \Sum \limits_{i=0}^n (-1)^i e_{q_i}.
		\end{equation*}
	\end{proof}
