...
\begin{document}
	...
	\begin{sketch}
		...
		Complex at $H_\bullet(X,B)$:
		\begin{tab}
			The composite $H_\bullet(A,B) --> H_\bullet(X,B) --> H_\bullet(X,A)$ factors through $H_\bullet(A,A) = 0$.
		\end{tab}

		Exactness at $H_\bullet(A,B)$: Consider $x \in \ker H_\bullet(i)$, find by using exactness of the corresponding long exact sequences elements $y,z,w,v$ via
		\begin{equation*}
			\begin{diagram}
	...
	Moreover, given a $\Z$-graded abelian group $M$, considered as a chain complex with zero differential, we obtain a short exact sequence of chain complexes
	\begin{align*}
	...
	\begin{definition}
		...
		\begin{equation*}
			\begin{array}{rcl}
				...
				{[f:(X,A)\rightarrow(Y,B)]} & \longmapsto & {[C^\text{sing}(f) \tensor M: C^\text{sing}(X,A) \tensor M \rightarrow C^\text{sing}(X,A) \tensor M]}
			\end{array}
		\end{equation*}
	...
	\newpage
	\subsection{Filtration, Künneth Formula and the Universal Coefficient Theorem}
	...
	\begin{lemma}[Filtration]
		...
		\begin{equation*}
			\colim \limits_{i \in \N_0} H^\text{sing}(U_i,A \,| M) \longrightarrow H^\text{sing}(X,A \,| M)
		\end{equation*}
		is an isomorphism.
	\end{lemma}

	\TODO{Künneth formular}
