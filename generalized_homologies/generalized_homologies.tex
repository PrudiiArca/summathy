...
\begin{document}
	...
	\begin{example}[Homology of Surfaces of Genus $g$]
		...
		\end{align*}
		For $\Delta_1 = \partial_1$ consider the diagram
		\begin{equation*}
			\begin{diagram}
				\threebytwo[verywide]
			...
			\end{diagram}
		\end{equation*}
		We know from previous calculations that we can chose isomorphisms $H_1(\D^1,\S^0) \isom R$ and $H_0(\S^0) \isom R \oplus R$ such that $\partial_1(x) = (x,-x)$ and $a(y_1,y_2) = y_1 + y_2$. Hence $a\partial_1 = 0$ and consequently $\Delta_1 = 0$, since it is on every component.
		% \begin{equation*}
		% 	\begin{diagram}
		% 		\matrix[objects,wide]{
		% 			\node (nwww) {$H_1(\S^1,*)$};\&
		...
		% 			\node (ne) {};\&
		% 			\node (nee) {};\&
		% 			\node (neee) {$H_0(\Coproduct \limits_{k=1}^{2g}*)$};\\
		% 			\node (swww) {$H_1(\Join \limits_{k=1}^{2g}\S^1,*)$};\&
		% 			\node (sww) {};\&
		% 			\node (sw) {$H_1(\Join \limits_{k=1}^{2g}\S^1,*)$};\&
		% 			\node (s) {$H_0(*)$};\&
		% 			\node (se) {$H_0(*)$};\&
		% 			\node (see) {};\&
		% 			\node (seee) {$H_0(\Coproduct \limits_{k=1}^{2g}*)$};\\
		% 		};
		% 	\end{diagram}
		% \end{equation*}
	\end{example}
