				...
				\begin{enumerate}
					...
					\item[(IS)]{
						...
						\begin{tab}[1.3cm]
							...
							A contradiction.
						\end{tab}
						\TODO{if we can replace $\D^k$ with homeomorphic things, why not with homotopy equivalent things like a point, in which case this would be trivial?}
						% \begin{align*}
						% 	\hspace{-3cm}...\widetilde{H}_{m+1}^\text{sing}(A_1 \union A_2) --> \widetilde{H}_m^\text{sing}(A_1) --> \widetilde{H}_m^\text{sing}(A_1 \union A_2) \oplus \widetilde{H}_m^\text{sing}(A_2) --> \widetilde{H}_m^\text{sing}(A_1 \union A_2)...
						% \end{align*}
					}
				\end{enumerate}
			}
