		...
		\begin{minipage}{4cm}
			\begin{center}
				\begin{tikzpicture}
					...
					\node at (1.5,0) {$\widetilde{\Sigma}(X,x_0)$};
				\end{tikzpicture}
			\end{center}
		\end{minipage}

		These constructions give functors $\Sigma: \ncat{Top} --> \ncat{Top}$ and $\widetilde{\Sigma}: \ncat{Top}_{*} --> \ncat{Top}_{*}$.
	\end{definition}

	\begin{lemma}[Properties of Suspension]
		There is a canonical isomorphism $\Sigma \S^n \isom \S^{n+1}$.
	\end{lemma}
