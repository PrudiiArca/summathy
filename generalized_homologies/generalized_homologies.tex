...
\begin{document}
	...
	\begin{definition}
		...
		\begin{equation*}
			...
			\end{array}
		\end{equation*}
		\TODO{homotopy extension property}
	\end{definition}
	...
	\begin{proposition}
		...
		\TODO{see AGPS algebraic topology from a homotopical viewpoint thm 4.1.14}
	\end{proposition}
	...
	\end{lemma}
	\begin{sketch}
		\TODO{Use compactness of $I$ and the fact that colimits commute.}
	...
	\begin{sketch}
		...
		% By assumption $[i,i+1]\times X_i \union \{i+1\}\times X$ is a deformation retract of $[i,i+1]\times X$ with given homotopy $H_i$. Extending this by the identity makes $([0,i+1] \times X \intersection \Tel X) \union ([i+1,\infty) \times X)$ a deformation retract of $[0,\infty) \times X$ via retractions $r_i$ and homotopies $K_i$.
		% Now let $r:[0,\infty) \times X --> \Tel X$ be the \underline{infinite} composite of the retractions.
	\end{sketch}
	...
	\begin{corollary}[Relative Mayer Vietories]
		Let $(H_\bullet,\partial_\bullet)$ be a generalized homology theory, $(X,B)$ a pair of spaces and $A_1,A_2 \subseteq X$ with $X \subseteq \interior(A_1) \union \interior(A_2)$. Then there is a long exact sequence
		\begin{equation*}
			\begin{diagram}
				\threebythree[hyperwide]
					{}{}{...H_{\bullet+1}(X,B)}
					{H_\bullet(A_1 \intersection A_2,A_1 \intersection A_2 \intersection B)}{H_\bullet(A_1,A_1 \intersection B)\directsum H_\bullet(A_2,A_2 \intersection B)}{H_\bullet(X,B)}
					{H_{\bullet-1}(A_1 \intersection A_2, A_1 \intersection A_2 \intersection B)...}{}{}
				...
				\draw[->,rounded corners] (ne.east) -| node[auto,pos=.7]{$\nu_{\bullet+1}$}($(ne)+(1.5,-.75)$) -| ($(w)+(-3,0)$) |- (w.west);
				\draw[->,rounded corners] (e.east) -| node[auto,pos=.7]{$\nu_\bullet$}($(e)+(1.5,-.75)$) -| ($(sw)+(-3,0)$) |- (sw.west);
			\end{diagram}
		\end{equation*}
	\end{corollary}
	\begin{sketch}
		\TODO{corollary of MV or does it require reduced homology?}
	\end{sketch}

	\begin{corollary}[Subspace Mayer Vietories]
		Let $(H_\bullet,\partial_\bullet)$ be a generalized homology theory, $(X,A)$ a pair of spaces and $A_1,A_2 \subseteq A$ with $A \subseteq \interior(A_1) \union \interior(A_2)$. Then there is a long exact sequence
		\begin{equation*}
			\begin{diagram}
				\threebythree[hyperwide]
					{}{}{...H_{\bullet+1}(X,A)}
					{H_\bullet(X,A_1 \intersection A_2)}{H_\bullet(X,A_1)\directsum H_\bullet(X,A_2)}{H_\bullet(X,A)}
					{H_{\bullet-1}(X, A_1 \intersection A_2)...}{}{}
	...
	\begin{lemma}[Homology of Mapping Telescope]
	...
	\end{lemma}
	\begin{sketch}
		Applying Mayer Vietories to \TODO{an $\varepsilon$-enlargement of} the decomposition
		\begin{align*}
		...
		\begin{align*}
			...
			\gamma(x_n) &= (-1)^n x_n
		\end{align*}
		From algebra (\TODO{see May p 115}) we know that the sequence
		\begin{equation*}
	...
	\newpage
	\subsection{Homology of the Sphere and Applications}
	...
	\begin{lemma}
		...
		Then $GL_n(\R)$ acts on $H_n(\R^n,\R^n\setminus \{0\})$ via $A \bullet x = \chi_R(\sign \det A) \cdot x$, where $\chi_R:\Z --> R$.

		In particular, if $f:\R^n --> \R^n$ is differentiable with $f(0)=0$ and $df(0) \in GL_n(\R)$, then $H_n(f)$ acts on $H_n(\R^n,\R^n\setminus \{0\})$ by multiplication with $\sign \det df(0)$.
	\end{lemma}
	\begin{sketch}
		$GL_n(\R) \subseteq \R^{n\times n}$ has two components distinguished by $\sign \det: GL_n(\R) --> \{\pm 1\}$, i.e. there is either a path from $A$ to $\diag(1,...,1)$ or from $A$ to $\diag(-1,1,...,1)$. These translate into homotopies from $A:\R^n --> \R^n$ to the identity or to a reflection (which restrict to maps $\S^{n-1} --> \S^{n-1}$), so $H_n(a)(x) = \chi_R(\sign \det A) \cdot x$.

		Note that $f \sim df(0)$ via the homotopy
		\begin{equation*}
			\begin{array}{rcl}
				h: \I \times \R^n & \longrightarrow & \R^n\\
				(t,x)\; & \longmapsto & (1-t)\left(\dfrac{f(te_1)}{t} \;\bigg|\; ... \;\bigg|\; \dfrac{f(te_n)}{t}\right)x + tf(x)
			\end{array}
		\end{equation*}
	\end{sketch}
	...
	Note that a manifold with fundamental class must satisfy $H_n(X) \neq 0$ by definition. Furthermore a manifold with fundamental class is by construction $R$-orientable. Indeed in the following special case both notions are equivalent.
	...
	\begin{sketch}
		...
		\end{equation*}
		Now if $H_n(X,X\setminus B_1)$ and $H_n(X,X\setminus B_2)$ are isomorphic to $R$, there are \TODO{is this doable?}
	\end{sketch}

	\begin{lemma}
		\TODO{TODO uniqueness of fundamental class for pathconnected...}
	\end{lemma}
	\begin{sketch}
		\TODO{show that there is a homotopy equivalence $X\setminus \{x\} \simeq X \setminus \{x'\}$ in such a way that for each $t$ we have $h(t,-):X\setminus\{x_t\} \longrightincl X$ (isotopy) by twisting points within unit spheres iteratively along a $x$-$x'$-path}
	\end{sketch}

	\begin{lemma}
		Let $(H_\bullet,\partial_\bullet)$ be an $R$-homology theory and $X$ be a hausdorff space.

		Given a discrete closed subset $S$ of $X$, the canonical morphism $\smash{H_n(X,X\setminus S) --> \Product \limits_{s \in S} H_n(X,X\setminus\{s\})}$ factors via $\Oplus \limits_{s \in S}H_n(X,X\setminus\{s\})$. 

		In particular, if $X$ is a topological $n$-manifold, which admits a fundamental class, then $S$ is finite.
	\end{lemma}
	\begin{sketch}
		As $X$ is hausdorff we find pairwise disjoint open neighborhoods $(U_s)_s$ of the elements $s$ in $S$ and we define $U = \Union_s U_s$. By excission, additivity and again excission we obtain a composite isomorphism
		\begin{align*}
			H_n(X,X\setminus S) \isom H_n(U,U\setminus S) \isom \Oplus \limits_{s \in S} H_n(U_s,U_s \setminus \{s\}) \isom \Oplus \limits_{s \in S} H_n(X,X\setminus \{s\}).
		\end{align*}
		Postcomposing with the inclusion $\Oplus \rightincl \Product$ \TODO{yields the canonical morphism}.

		For a manifold with fundamental class $[X]$ we find that the fundamental class maps to $(1)_s \in \Oplus_s H_n(X,X\setminus \{s\})$, so $S$ has to be finite.
	\end{sketch}

	\newpage
	\subsection{Local Degree, Degree and Applications}
	...
	\begin{remark}[Local Degree of Locally Differentiable Maps]
		Let $(H_\bullet, \partial_\bullet)$ be an $R$-homology theory and $f:X-->Y$ be a continuous map between topological $n$-manifolds. Fix a point $x$ in $X$ and isomorphisms $R \isom H_n(X,X\setminus\{x\})$ and $R \isom H_n(Y,Y\setminus \{f(x)\})$.

		Suppose that $f$ is locally differentiable in $x$, meaning that there is an euclidean neighborhood $V \isom \R^n$ of $f(x)$ in $Y$ with $f(x) \corres 0$ and an euclidean neighborhood $U \isom \R^n$ of $x$ with $x \corres 0$ and $f(U) \subseteq V$ such that the restricted map $g=f\vert_U^V:\R^n --> \R^n$ is differentiable in $0$. Suppose further that $U \intersection f^{-1}(\{f(x)\}) = \{x\}$.

		We can choose an isomorphism $\phi:H_n(\R^n,\R^n\setminus \{0\}) \isom R$ such that the following diagram commutes.
		\begin{equation*}
			\begin{diagram}
				\fourbythree[hyperwide]
					{R\isom H_n(X,X\setminus\{x\})}{H_n(U,U\setminus\{x\})}{H_n(V,V\setminus\{x\})}{H_n(Y,Y\setminus\{f(x)\})\isom R}
					{}{H_n(\R^n,\R^n\setminus\{0\})}{H_n(\R^n,\R^n\setminus\{0\})}{}
					{}{R}{R}{}

				\arrow[out=10,in=170]{nww}{nee}{H_n(f)}[above]
				\arrow{nw}{nww}{\isom}[above]
				\arrow{nw}{ne}{}
				\arrow{ne}{nee}{\isom}[above]
				\arrow{w}{e}{H_n(g)}[above]
				\arrow{sw}{se}{}

				\arrow{nw}{w}{\isom}[left]
				\arrow{w}{sw}{\isom \;\; \phi}

				\arrow{ne}{e}{\isom}[right]
				\arrow{e}{se}{\phi\;\;\isom}

				\arrow[equals]{$(nww)+(-1.25,-.3)$}{sw}{}
				\arrow{se}{$(nee)+(1.25,-.3)$}{\isom}[below right]
			\end{diagram}
		\end{equation*}
		The right diagonal isomorphism maps $1$ to some unit $\alpha$. By a previous lemma the lowest horizontal morphism maps $1$ to $\sign \det dg(0)$. Hence we get
		\begin{equation*}
			\deg_{R,x}(f) = \alpha \cdot \sign \det dg(0).
		\end{equation*}
		Note that we may get for different choices of $U$ and $V$ the identity $\deg_{R,x}(f) = \alpha' \cdot \sign \det dg'(0)$, however, since $\deg_{R,x}(f)$ is independent from the choice of $U$ and $V$, we will find that $\alpha \cdot \sign \det dg(0) = \alpha' \cdot \sign \det dg'(0)$.
	\end{remark}

	While the local $R$-degree can be defined for maps between arbitrary $n$-manifolds, in the presence of fundamental classes a global notion of degree can be obtained, which generalizes the degree of maps of spheres.

	\begin{definition}
		Let $(H_\bullet,\partial_\bullet)$ be an $R$-homology theory and $f:X-->Y$ be a continuous function of topological $n$-manifolds, which admit fundamental classes $[X]$ and $[Y]$ respectively. For each $y \in Y$ fix an isomorphism $H_n(Y,Y\setminus\{y\})\isom R$ in such a way that $[Y]$ maps to $1$ under the map $H_n(Y) --> H_n(Y,Y\setminus\{y\}) \isom R$.

		Suppose that $Y$ is connected and that $y$ is any point in $Y$. The \textbf{$R$-degree} of $f$ is defined to be the element $\deg_R(f)$ in $R$ to which $[X]$ is mapped under the morphism
		\begin{equation*}
			H_n(X) \longrightarrow H_n(Y) \longrightarrow H_n(Y,Y\setminus \{y\}) \isom R.
		\end{equation*}
		This is independent from the choice of $y$, since by the continuation lemma for points $y,y'$ in the same continuation neighborhood $V \subseteq Y$ there is a commuting diagram
		\begin{equation*}
			\begin{diagram}
				\fivebythree[verywide]
					{}{}{}{H_n(Y,Y\setminus \{y\})}{R}
					{H_n(X)}{H_n(Y)}{H_n(Y,Y\setminus V)}{}{}
					{}{}{}{H_n(Y,Y\setminus \{y'\})}{R}

				\arrow{ww}{w}{H_n(f)}[above]
				\arrow{w}{c}{}

				\arrow[bend left]{w}{ne}{}
				\arrow[bend right]{w}{se}{}

				\arrow{c}{ne}{}
				\arrow{c}{se}{}

				\arrow{ne}{nee}{\isom}[above]
				\arrow{se}{see}{\isom}[below]

				\arrow[equals]{nee}{see}{}
			\end{diagram}
		\end{equation*}
		and this argument can be applied inductively along a path connecting arbitrary points $y,y'$ in $Y$.
	\end{definition}

	The following proposition connects the global and the local notion of degree.

	\begin{proposition}
		Let $(H_\bullet,\partial_\bullet)$ be an $R$-homology theory and $f:X --> Y$ be a continuous map between topological $n$-manifolds $X$ and $Y$ with fundamental classes $[X]$ and $[Y]$ respectively. Let $y$ be in $Y$ and suppose that $Y$ is connected and that $f^{-1}(\{y\})$ is a nonempty discrete subset of $X$.

		For $x \in f^{-1}(\{y\})$ fix isomorphisms $H_n(X,X\setminus\{x\}) \isom R$ such that $[X]$ maps to $1$. Further fix an isomorphism $H_n(Y,Y\setminus y) \isom R$ with $[Y]$ mapping to $1$. Then
		\begin{equation*}
			\deg_R(f) = \Sum \limits_{x \in f^{-1}(\{y\})} \deg_{R,x}(f).
		\end{equation*}
	\end{proposition}
	\begin{sketch}
		Follows by chasing $[X]$ through the commuting diagram
		\begin{equation*}
			\begin{diagram}
				\threebythree[ultrawide]
					{H_n(X)}{}{\Oplus \limits_{x \in f^{-1}(\{y\})} H_n(X,X\setminus \{x\})}
					{}{H_n(X,X\setminus f^{-1}(\{y\}))}{}
					{H_n(Y)}{}{H_n(Y,Y\setminus\{y\})}

				\arrow{nw}{ne}{\Oplus \iota_x}[above]
				\arrow{sw}{se}{}

				\arrow{nw}{sw}{H_n(f)}[left]
				\arrow{ne}{se}{\Oplus H_n(f)}[right]

				\arrow{nw}{c}{}
				\arrow{c}{se}{H_n(f)}[ontop]
				\arrow{c}{$(ne)+(-1,-.75)$}{\isom}[ontop]
			\end{diagram}
		\end{equation*}
	\end{sketch}
	...
	\end{corollary}
	\begin{sketch}
		Extend $f$ to a function $f':\R --> \R$ by setting $f'(x) = x$ outside $[0,1]$. Then extend it further to a function $f'': \S^1 --> \S^1$. Now $\deg_R(f'') = \deg_{R,\infty}(f'') = 1$, so $f''$ is surjective. By construction this implies that $f$ is surjective.
	\end{sketch}

	% For the next application note that given any smooth function $f:\R^n --> \R^n$ with $f(0)=0$ is homotopic to its derivative $f':\R^n --> \R^n$ via the homotopy
	% \begin{equation*}

	% \end{equation*}

	\begin{proposition}
		Let $(H_\bullet, \partial_\bullet)$ be a $\Z$-homology theory and $f:\R^n --> \R^n$ a smooth function with $f(0)=0$. Then $H_n(f)$ acts on $H_n(\R^n,\R^n \setminus \{0\})$ by multiplication with $\sign \det df(0)$.
	\end{proposition}
	...
	\begin{corollary}[Homology of CW-complexes]
		...
		\begin{enumerate}[(i)]
			\item{
				We have $H_\bullet(X_n,X_{n-1}) \isom H_\bullet(X_n/X_{n-1},*) \isom H_\bullet(\Join \limits_{i \in I_n} \S^n) \isom \Oplus \limits_{i \in I_n}H_\bullet(\S^n) \isom \Directsum \limits_{i \in I_n}M_{\bullet-n}$.% and in particular
				% \begin{equation*}
	...
	\begin{theorem}[Uniqueness of Cellular Homology]
	...
	\begin{remark}
		For any CW-complex $(X,A)$ with given CW-structure, we have \TODO{$R$ noetherian / PID sufficient?} 
		\begin{equation*}
	...
	In the following we fix an $R$-homology theory $(H_\bullet,\partial_\bullet)$.

	\begin{example}[Homology of the Sphere revisited]
	...
	\begin{example}[Homology of Surfaces of Genus $g$][nobreak=false]
		...
		\begin{equation*}
			...
			\end{diagram},
		\end{equation*}
		where the attaching map $\chi$ is given by the gluing scheme \TODO{todo}. Hence the cellular complex is given by
		\begin{equation*}
	...
	\begin{lemma}[Homology of Real Projective Spaces]
		TODO
	\end{lemma}

	\begin{lemma}[Homology of Complex Projective Spaces]
		TODO
	\end{lemma}

	\begin{lemma}[Homology of Moore Spaces]
		TODO
	\end{lemma}
	...
	\begin{corollary}[Künneth Formula]
		...
		\end{align*}
		which is natural in $X$ and $Y$ and splits (in a not natural way) \TODO{and compatible with boundaries?}
	\end{corollary}
	\begin{proof}
	...
	\begin{sketch}
		\begin{enumerate}[(i)]
			\item{
				The composite $\S^n \longrightincl \S^m \longrightincl \S^n$ is by the previous corollary an homeomorphism. Hence $\S^m \longrightincl \S^n$ is by the compact hausdorff trick a homeomorphism and by invariance of dimension $m = n$.
			}
			\item{
	...
	One can show that singular homology with coefficients in $M_\bullet$ is again a (generalized, if $M_\bullet$ is not concentrated in degree zero) additive homology theory, satisfying the filtration lemma, either by adapting the proofs \TODO{or by applying the Künneth-formula of homological algebra???}.
	...
	\begin{theorem}[Universal Coefficient Theorem]
		...
		\begin{align*}
			0 --> H_n^\text{sing}(X,A) \tensor G --> H_n^\text{sing}(X,A \mid G) --> \Tor(H_{n-1}^\text{sing}(X,A),G)-->0,
		\end{align*}
