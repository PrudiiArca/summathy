...
\begin{document}
	...
	\begin{definition}
		A functor, which is isomorphic to one of the form $V(F,-)$, is called an \textbf{affine scheme over $R$}. More precisely an affine scheme over $R$ is a functor $\ncat{Alg}_R \rightarrow \ncat{Set}$, which is representable by an $R$-algebra of finite presentation. A \textbf{morphism of affine schemes over $R$} is a mere natural transformation. We write $\ncat{Aff}_R$ for the corresponding full subcategory of the functor category $|[\ncat{Alg}_R,\ncat{Set}|]$.
	\end{definition}
	...
	\begin{example}
		...
		\begin{enumerate}[$(i)$]
			...
			\item{
				...
				\end{array}$ is represented by $R[X,Y]/(XY-1)$.
			}
			\item{
				The functor $\begin{array}{rcl}
	...
	\end{equation*}
	As such $\ncat{Aff}_R$ exhibits the same categorical structure as $(\ncat{Alg}_R^\fp)^\op$, in particular it is \TODO{finitely complete and finitely cocomplete}.
