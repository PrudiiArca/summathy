...
\begin{document}
	...
	\begin{definition}
		A \textbf{zero divisor} in a ring $R$ is an element $a$, such that there exists a nonzero element $b$ such that
		\begin{equation*}
			ab = 0 \;\;\;\;\text{or}\;\;\;\; ba = 0.
		\end{equation*}
		\TODO{nilpotent}
		...
		A commutative ring, which does not possess any zero divisors besides $0$, is called an \textbf{integral domain}.
	\end{definition}

	\TODO{0 zero divisor unless zero ring}

	\begin{definition}
		A \textbf{unit} in a ring $R$ is an element $a$, for which there exists a \textbf{multiplicative inverse}, i.e. an element $a^{-1}$ satisfying
		\begin{equation*}
			aa^{-1} = 1 = a^{-1}a.
		\end{equation*}
		The subset of all units in $R$ will be denoted by $R^\times$. 

		A commutative ring, in which every element except $0$ is a unit, is a \textbf{field}.
	\end{definition}
