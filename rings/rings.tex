...
\begin{document}
	...
	\begin{lemma}[Properties of $\ncat{CRing}$]
		...
		\begin{enumerate}[(i)]
			...
			\item{
				\TODO{A morphism of rings $f:R --> S$ is epi if and only if it is surjective. WRONG see localization}
			}
			\item{
			...
			\item{
				...
				\end{equation*}
				\TODO{given by the subring $\{r \in R \mid f_1(r) = f_2(r)\}$ wrong?} and its inclusion. Thus $\ncat{CRing}$ is \textit{complete}.
			}
		\end{enumerate}
	\end{lemma}
	\begin{sketch}
	...
	\begin{sketch}
		...
		(iii)$\implies$(i) Since in an integral domain every prime element is irreducible, it suffices to show uniqueness of the factorization. So given two factorizations with $m \leq n$
		\begin{equation*}
			u\cdot p_1\cdot ...\cdot p_m = v \cdot q_1 \cdot ... \cdot q_n
		\end{equation*}
		we find that $p_i \divides q_{j_i}$
	\end{sketch}
